...
abstract class Pad_Meta_Box {
	 ...
	 */
	protected function post_select( $label, $selection, $name, $post_type, $class = "", $meta_fields = array(), $meta_field_label_map = '') {
		...
		while ( $post_query->have_posts() ) : $post_query->the_post();
			...
			if ( count($meta_fields) > 0 ) {
				...
				foreach ( $meta_fields as $meta_field ) {
					if ( !empty( $meta_field_label_map) && $meta_field == $meta_field_label_map ) {
						// Metafield must be the id for the mapped post.
						$mapped_id = get_post_meta( $id, $meta_field, true);
						$mapped_title = get_the_title( $mapped_id );
						$option_label = $mapped_title;
					}
					else {
